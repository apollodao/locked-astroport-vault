...
use cw_it::{robot::TestRobot, test_tube::Account, traits::CwItRunner};
...
fn test_compound_vault() {
    ...
    // Donate some reward tokens to the vault to simulate rewards accruing, then compound and check
    // that the base token amount corresponding to the users vault token balance has increased.
    ...
    for token in reward_tokens {
        println!("Donating {}", token.to_string());
