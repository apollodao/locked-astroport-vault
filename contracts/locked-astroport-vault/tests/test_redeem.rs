...
#[test]
fn withdrawal_fee_with_multiple_recipients_works_without_lockup() {
    let owned_runner = get_test_runner();
    let runner = owned_runner.as_ref();
    let admin = runner
        .init_account(&Coins::from_str(DEFAULT_COINS).unwrap().to_vec())
...
#[test]
fn withdrawal_fee_with_multiple_recipients_works_with_lockup() {
    let owned_runner = get_test_runner();
    let runner = owned_runner.as_ref();
    let admin = runner
        .init_account(&Coins::from_str(DEFAULT_COINS).unwrap().to_vec())
    ...
    let withdrawal_fee = Some(FeeConfig {
        fee_rate,
        fee_recipients: vec![
            (treasury_one.address(), Decimal::percent(30)),
            (treasury_two.address(), Decimal::percent(70)),
        ],
    });
    let robot =
        instantiate_wsteth_eth_vault(&runner, &admin, None, None, withdrawal_fee, &dependencies);
    ...
    robot
        ...
        .assert_base_token_balance_eq(
            treasury_one.address(),
            deposit_amount * fee_rate * Decimal::percent(30),
        )
        .assert_base_token_balance_eq(
            treasury_two.address(),
            deposit_amount * fee_rate * Decimal::percent(70),
        );
}
